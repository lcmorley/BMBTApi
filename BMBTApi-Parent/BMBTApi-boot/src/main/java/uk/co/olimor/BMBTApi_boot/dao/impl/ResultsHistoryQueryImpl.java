...
public class ResultsHistoryQueryImpl extends AbstractQuery<TestResult> implements ResultHistoryQuery {
	 ...
	 * Query.
	 */
	private static final String QUERY = "SELECT * FROM testResult WHERE userId = '%s' AND testType = 'Full' "
			+ "ORDER BY testId ASC";
	...
	protected List<TestResult> buildResult(ResultSet result) throws SQLException {
		...
		try {
			while (result.next()) 
				results.add(new TestResult(result.getString(2), result.getInt(3), result.getInt(4), result.getInt(5), 
					 result.getFloat(6), result.getString(7)));			
